...
public class ChangeTownNameCasing {
    ...
    private static final String UPDATE_TOWNS_FROM_COUNTRY_TO_UPPERCASE_SQL =
            "UPDATE `towns` " +
                    "SET `name` = UPPER(`name`) " +
                    "WHERE `country` = ?";
    ...
    public static void main(String[] args) {
        ...
        try (Connection connection = DriverManager.getConnection(CONNECTION_URL + SCHEMA_NAME, props);
             PreparedStatement changeTownNameToUppercase = connection.prepareStatement(UPDATE_TOWNS_FROM_COUNTRY_TO_UPPERCASE_SQL);
            ...
            int affectedTowns = changeTownNameToUppercase.executeUpdate();
            ...
            if (!rs.next()) {
                ...
                System.out.printf("%d town names were affected.%n" +
                        "[%s]", affectedTowns, String.join(", ", updatedTowns));
            }
